...
const App = () => {
  ...
  const API_KEY= 'fd66a71d286c1637437f0f1079cbbe53'
  ...
  // Fetches 5 location suggestion from API and updates
  ...
  // This function checks  if weather exists or not and returns an object
     ...
     <div className='flex items-center justify-center p-6 min-h-screen'>
      ...
      {!weather ? (
        <form onSubmit={handleSearch} className='flex flex-col relative'>
          <input value={city} onChange={(e) => setCity(e.target.value)} placeholder = 'Enter city or Country (min 3 letters)' className='mb-4 p-3 rounded border border-white bg-transparent text-white placeholder-white focus:outline-none focus:border-blue-300 transition duration-300' />
          {suggestion.length > 0 && (
        ...
        <div className='mt-6 text-center transition-opacity duration-500'>
          <button onClick={() => {_setweather(null); setCity('')}}
